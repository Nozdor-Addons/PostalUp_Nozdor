...
local Postalup_Select = Postalup:NewModule("Selectup", "AceEvent-3.0", "AceHook-3.0")
local L = LibStub("AceLocale-3.0"):GetLocale("Postalup")
Postalup_Select.description = L["Add check boxes to the inbox for multiple mail operations."]
Postalup_Select.description2 = L[ [[|cFFFFCC00*|r Selected mail will be batch opened or returned to sender by clicking Open or Return.
|cFFFFCC00*|r You can Shift-Click 2 checkboxes to mass Selectup every mail between the 2 checkboxes.
|cFFFFCC00*|r You can Ctrl-Click a checkbox to mass Selectup or unselect every mail from that sender.
|cFFFFCC00*|r Selectup will never delete any mail (mail without text is auto-deleted by the game when all attached items and gold are taken).
|cFFFFCC00*|r Selectup will skip CoD mails and mails from Blizzard.
...
local checkboxFunc = function(self) Postalup_Select:ToggleMail(self) end
...
updateFrame:SetScript("OnShow", function(self)
	self.time = Postalup.db.profile.OpenSpeed
end)
updateFrame:SetScript("OnUpdate", function(self, elapsed)
	self.time = self.time - elapsed
	if self.time <= 0 then
		self:Hide()
		Postalup_Select:ProcessNext()
	end
end)
...
local function printTooMuchMail()
	...
	if cur>=50 then
		Postalup:Print(format(L["There are %i more messages not currently shown."], lastUnseen))
	else
		Postalup:Print(format(L["There are %i more messages not currently shown. More should become available in %i seconds."], lastUnseen, lastTime+61-GetTime()))
	end
...
function Postalup_Select:OnEnable()
	--create the open button
	if not openButton then
		openButton = CreateFrame("Button", "PostalupSelectOpenButton", InboxFrame, "UIPanelButtonTemplate")
		openButton:SetWidth(120)
		openButton:SetHeight(25)
		openButton:SetPoint("RIGHT", InboxFrame, "TOP", 5, -53)
		openButton:SetText(L["Open"])
		openButton:SetScript("OnClick", function() Postalup_Select:HandleSelect(1) end)
		openButton:SetFrameLevel(openButton:GetFrameLevel() + 1)
	end

	--create the return button
	if not returnButton then
		returnButton = CreateFrame("Button", "PostalupSelectReturnButton", InboxFrame, "UIPanelButtonTemplate")
		...
		returnButton:SetScript("OnClick", function() Postalup_Select:HandleSelect() end)
	...
	for i = 1, 7 do
		if not _G["PostalupInboxCB"..i] then
			local CB = CreateFrame("CheckButton", "PostalupInboxCB"..i, _G["MailItem"..i], "OptionsCheckButtonTemplate")
			CB:SetID(i)
			...
			local text = CB:CreateFontString("PostalupInboxCB"..i.."Text", "BACKGROUND", "GameFontHighlightSmall")
    ...
    if InboxTooMuch then
        InboxTooMuch.Show = printTooMuchMail
        InboxTooMuch:Hide()
    end
	...
	returnButton:Show()
	for i = 1, 7 do
		_G["PostalupInboxCB"..i]:Show()
	end
end

function Postalup_Select:OnDisable()
	self:Reset()
...
function Postalup_Select:ToggleMail(frame)
	...
	if IsControlKeyDown() then
		-- Sneaky feature to ctrl-click a checkbox to Selectup/unselect every
		-- mail where the sender is the same
		local status = frame:GetChecked()
		local indexSender = Selectup(3, GetInboxHeaderInfo(index))
		for i = 1, GetInboxNumItems() do
			if Selectup(3, GetInboxHeaderInfo(i)) == indexSender then
...
function Postalup_Select:HandleSelect(mode)
...
function Postalup_Select:ProcessNext()
	...
	if mailIndex > 0 then
		local msgSubject, msgMoney, msgCOD, _, msgItem, _, wasReturned, msgText, canReply, isGM = Selectup(4, GetInboxHeaderInfo(mailIndex))
		...
		if currentMode then
			...
			if wait then
				local attachCount, goldCount = Postalup:CountItemsAndMoney()
			...
			if Postalup.db.profile.Selectup.SpamChat and attachIndex == ATTACHMENTS_MAX_RECEIVE then
				if not invFull or msgMoney > 0 then
					local moneyString = msgMoney > 0 and " ["..Postalup:GetMoneyString(msgMoney).."]" or ""
					Postalup:Print(format("%s %d: %s%s", L["Open"], mailIndex, msgSubject or "", moneyString))
				end
			end
			...
			if attachIndex > 0 and not invFull and Postalup.db.profile.Selectup.KeepFreeSpace > 0 then
				local free = 0
				...
				if free <= Postalup.db.profile.Selectup.KeepFreeSpace then
					invFull = true
					Postalup:Print(format(L["Not taking more items as there are now only %d regular bagslots free."], free))
				end
			end

			if attachIndex > 0 and not invFull then
				-- If there's attachments, take the item
				--Postalup:Print("Getting Item from Message "..mailIndex..", "..attachIndex)
				TakeInboxItem(mailIndex, attachIndex)

				lastNumAttach, lastNumGold = Postalup:CountItemsAndMoney()
				wait = true
				-- Find next attachment index backwards
			...
			elseif msgMoney > 0 then
				...
				--Postalup:Print("Getting Gold from Message "..mailIndex)
			...
			-- Return mode
			if Postalup.db.profile.Selectup.SpamChat and attachIndex == ATTACHMENTS_MAX_RECEIVE then
				Postalup:Print(L["Return"].." "..mailIndex..": "..msgSubject)
			end
			if not wasReturned and canReply then
				...
				Postalup:Print(L["Skipping"].." "..mailIndex..": "..msgSubject)
				mailIndex = mailIndex - 1
				return self:ProcessNext() -- tail call
			end
		end
		...
		if skipFlag then Postalup:Print(L["Some Messages May Have Been Skipped."]) end
		self:Reset()
	end
end

function Postalup_Select:InboxFrame_Update()
...
function Postalup_Select:MAIL_INBOX_UPDATE()
	--Postalup:Print("update")
	self:UnregisterEvent("MAIL_INBOX_UPDATE")
	updateFrame:Show()
end

function Postalup_Select:Reset(event)
...
function Postalup_Select:UI_ERROR_MESSAGE(event, error_message)
...
function Postalup_Select.SetKeepFreeSpace(dropdownbutton, arg1, arg2, checked)
	Postalup.db.profile.Selectup.KeepFreeSpace = arg1
end

function Postalup_Select.ModuleMenu(self, level)
	...
	if level == 1 + self.levelAdjust then
		...
		info.arg1 = "Selectup"
		info.arg2 = "SpamChat"
		info.checked = Postalup.db.profile.Selectup.SpamChat
		info.isNotRadio = 1
		UIDropDownMenu_AddButton(info, level)

	elseif level == 2 + self.levelAdjust then
		if UIDROPDOWNMENU_MENU_VALUE == "KeepFreeSpace" then
			local keepFree = Postalup.db.profile.Selectup.KeepFreeSpace
			info.func = Postalup_Select.SetKeepFreeSpace
			for _, v in ipairs(Postalup.keepFreeOptions) do
				info.text = v
