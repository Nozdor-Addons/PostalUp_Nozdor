...
local Postalup_DoNotWant = Postalup:NewModule("DoNotWantup", "AceHook-3.0")
local L = LibStub("AceLocale-3.0"):GetLocale("Postalup")
Postalup_DoNotWant.description = L["Shows a clickable visual icon as to whether a mail will be returned or deleted on expiry."]
...
StaticPopupDialogs["Postalup_DELETE_MAIL"] = {
...
StaticPopupDialogs["Postalup_DELETE_MONEY"] = {
...
function Postalup_DoNotWant.Click(self, button, down)
	...
	if InboxItemCanDelete(selectedID) then
		if firstAttachName then
			StaticPopup_Show("Postalup_DELETE_MAIL", firstAttachName)
			return
		elseif money and money > 0 then
			StaticPopup_Show("Postalup_DELETE_MONEY")
			return
		else
			DeleteInboxItem(selectedID)
		end
...
function Postalup_DoNotWant:OnEnable()
	-- Create the icons
	for i = 1, 7 do
		...
		if not b.returnicon then
			...
			b.returnicon.id = i
			b.returnicon:SetScript("OnClick", Postalup_DoNotWant.Click)
...
function Postalup_DoNotWant:OnDisable()
...
function Postalup_DoNotWant:InboxFrame_Update()
