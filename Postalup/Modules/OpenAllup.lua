...
local Postalup_OpenAll = Postalup:NewModule("OpenAllup", "AceEvent-3.0")
local L = LibStub("AceLocale-3.0"):GetLocale("Postalup")
Postalup_OpenAll.description = L["A button that collects all attachments and coins from mail."]
Postalup_OpenAll.description2 = L[ [[|cFFFFCC00*|r Simple filters are available for various mail types.
|cFFFFCC00*|r Shift-Click the Open All button to override the filters and take ALL mail.
|cFFFFCC00*|r OpenAllup will never delete any mail (mail without text is auto-deleted by the game when all attached items and gold are taken).
|cFFFFCC00*|r OpenAllup will skip CoD mails and mails from Blizzard.
...
local wait
local button
local Postalup_OpenAllMenuButton
local skipFlag
local invFull
...
updateFrame:SetScript("OnUpdate", function(self, elapsed)
	...
	if self.time <= 0 then
		self:Hide()
		Postalup_OpenAll:ProcessNext()
	end
end)

local function HideOldPostalButton()
    local f = _G["PostalOpenAllButton"]
    local f2 = _G["Postal_BlackBookButton"]
    if f then
        f:SetScript("OnShow", f.Hide)
        f:Hide()
    end
	if f2 then
        f2:SetScript("OnShow", f2.Hide)
        f2:Hide()
    end
end
...
refreshFrame:SetScript("OnUpdate", function(self, elapsed)
	...
	if self.time <= 0 then
		self.time = 10
		Postalup:Print(L["Refreshing mailbox..."])
		CheckInbox()
		...
		if current == 50 or current == total then
			...
			self:Hide()
			Postalup_OpenAll:OpenAllup(true)
		end
	end
end)

function Postalup_OpenAll:OnEnable()
	HideOldPostalButton()
	if not button then
		button = CreateFrame("Button", "PostalupOpenAllButton", InboxFrame, "UIPanelButtonTemplate")
		button:SetWidth(120)
		button:SetHeight(25)
		...
		button:SetScript("OnClick", function() Postalup_OpenAll:OpenAllup() end)
		button:SetFrameLevel(button:GetFrameLevel() + 1)
	end
	if not Postalup_OpenAllMenuButton then
		-- Create the Menu Button
		Postalup_OpenAllMenuButton = CreateFrame("Button", "Postalup_OpenAllMenuButton", InboxFrame);
		Postalup_OpenAllMenuButton:SetWidth(30);
		Postalup_OpenAllMenuButton:SetHeight(30);
		Postalup_OpenAllMenuButton:SetPoint("LEFT", button, "RIGHT", -2, 0);
		Postalup_OpenAllMenuButton:SetNormalTexture("Interface\\ChatFrame\\UI-ChatIcon-ScrollDown-Up");
		Postalup_OpenAllMenuButton:SetHighlightTexture("Interface\\Buttons\\ButtonHilight-Round");
		Postalup_OpenAllMenuButton:SetDisabledTexture("Interface\\ChatFrame\\UI-ChatIcon-ScrollDown-Disabled");
		Postalup_OpenAllMenuButton:SetPushedTexture("Interface\\ChatFrame\\UI-ChatIcon-ScrollDown-Down");
		Postalup_OpenAllMenuButton:SetScript("OnClick", function(self, button, down)
			if Postalup_DropDownMenu.initialize ~= Postalup_OpenAll.ModuleMenu then
				CloseDropDownMenus()
				Postalup_DropDownMenu.initialize = Postalup_OpenAll.ModuleMenu
			end
			ToggleDropDownMenu(1, nil, Postalup_DropDownMenu, self:GetName(), 0, 0)
		end)
		Postalup_OpenAllMenuButton:SetFrameLevel(Postalup_OpenAllMenuButton:GetFrameLevel() + 1)
	end

	self:RegisterEvent("MAIL_SHOW")
	-- For enabling after a disable
	button:Show()
	Postalup_OpenAllMenuButton:SetScript("OnHide", Postalup_DropDownMenu.HideMenu)
	Postalup_OpenAllMenuButton:Show()
end

function Postalup_OpenAll:OnDisable()
	self:Reset()
	button:Hide()
	Postalup_OpenAllMenuButton:SetScript("OnHide", nil)
	Postalup_OpenAllMenuButton:Hide()
end

function Postalup_OpenAll:MAIL_SHOW()
	HideOldPostalButton()
	self:RegisterEvent("MAIL_CLOSED", "Reset")
	self:RegisterEvent("PLAYER_LEAVING_WORLD", "Reset")
end

function Postalup_OpenAll:OpenAllup(isRecursive)
	refreshFrame:Hide()
	-- Get mail counts
...
function Postalup_OpenAll:ProcessNext()
	if mailIndex > 0 then
		...
		local sender, msgSubject, msgMoney, msgCOD, _, msgItem, _, _, msgText, _, isGM = Selectup(3, GetInboxHeaderInfo(mailIndex))
		...
		-- Filter by mail type
		local mailType = Postalup:GetMailType(msgSubject)
		if mailType == "NonAHMail" then
			-- Skip player sent mail with attachments according to user options
			if not (openAllOverride or Postalup.db.profile.OpenAllup.Attachments) and msgItem then
				mailIndex = mailIndex - 1
				attachIndex = ATTACHMENTS_MAX_RECEIVE
				return self:ProcessNext() -- tail call
			end
			...
			if not (openAllOverride or Postalup.db.profile.OpenAllup[mailType]) then
				mailIndex = mailIndex - 1
				attachIndex = ATTACHMENTS_MAX_RECEIVE
				return self:ProcessNext() -- tail call
			end
		...
		if Postalup.db.profile.OpenAllup.SpamChat and attachIndex == ATTACHMENTS_MAX_RECEIVE then
			if not invFull or msgMoney > 0 then
				local moneyString = msgMoney > 0 and " ["..Postalup:GetMoneyString(msgMoney).."]" or ""
				Postalup:Print(format("%s %d: %s%s", L["Processing Message"], mailIndex, msgSubject or "", moneyString))
			end
		end
		...
		if attachIndex > 0 and not invFull and Postalup.db.profile.OpenAllup.KeepFreeSpace>0 then
			local free=0
			...
			if free <= Postalup.db.profile.OpenAllup.KeepFreeSpace then
				invFull = true
		...
		if origNumItems ~= numItems or origTotalItems ~= totalItems then
			...
			if (totalItems > numItems and numItems < 50) or (origTotalItems > origNumItems) then
				Postalup:Print(L["Not all messages are shown, refreshing mailbox soon to continue Open All..."])
				refreshFrame:Show()
				return
			end
		end
...
function Postalup_OpenAll:Reset(event)
	refreshFrame:Hide()
	updateFrame:Hide()
...
function Postalup_OpenAll:UI_ERROR_MESSAGE(event, error_message)
...
function Postalup_OpenAll.SetKeepFreeSpace(dropdownbutton, arg1)
	Postalup.db.profile.OpenAllup.KeepFreeSpace = arg1
end

function Postalup_OpenAll.ModuleMenu(self, level)
	if not level then return end
	local info = self.info
	wipe(info)
	info.isNotRadio = 1
	local db = Postalup.db.profile.OpenAllup
	...
	elseif level == 3 + self.levelAdjust then
		if UIDROPDOWNMENU_MENU_VALUE == "KeepFreeSpace" then
			...
			info.func = Postalup_OpenAll.SetKeepFreeSpace
			info.isNotRadio = nil
